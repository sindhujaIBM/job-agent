...
EventBridge Scheduler (7:30am MT daily)
  → job-scanner    — fetch WWR RSS + RemoteOK API + Remotive API + Calgary company boards, dedup, save new jobs to remote-jobs-scan-queue
...
| File | Purpose |
|---|---|
| `src/lib/jobFeeds.ts` | Fetch + normalize We Work Remotely RSS, RemoteOK API, Remotive API, and Calgary company Greenhouse/Lever boards |
