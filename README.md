...
A daily remote-job scanner and matcher. It pulls listings from **We Work Remotely**, **Remote OK**, and **Remotive**, scores each one against two independent fit profiles using Claude Haiku on AWS Bedrock, and emails a digest to each person when there's a match. Approving a match from the email kicks off automatic resume + cover letter generation in a companion repo — nothing is ever auto-submitted to an employer; a human always reviews the generated documents before applying.
...
EventBridge Scheduler (daily)
  -> job-scanner    fetch We Work Remotely + Remote OK + Remotive, dedup, queue new listings
